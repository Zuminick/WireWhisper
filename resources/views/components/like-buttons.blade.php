<div>
    <form method="POST" action="/whisps/{{ $whisp->id }}/like">
        @csrf

        <div
            class="flex justify-items-auto justify-between {{ $whisp->islikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500'}}">
            <button type="submit" class="text-xm px-10 {{ $whisp->likes ? '' : 'py-2' }}">
    ...
    <div class="mt-0">
        <form method="POST" action="/whisps/{{ $whisp->id }}/like">
            ...
            <div
                class="flex justify-items-auto {{ $whisp->isDislikedBy(current_user()) ? 'text-red-500' : 'text-gray-500'}}">
                <button type="submit" class="text-xm px-10 {{ $whisp->dislikes ? '' : 'py-2' }}">
                    <svg viewBox="0 0 20 20" class="mr-1 w-3">
                        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g class="fill-current">
                                <path
